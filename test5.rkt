...
(define (get-parent-node-offset node s p)
  (if (eq? node p)
      (vec3 0 0 0)
      (let* ((idx (get-node-index node))
	     (parent (get-parent-node node))
	...
	(if (eq? parent p)
	    r
	    (let ((pv (get-parent-node-offset parent (* s 2) p)))
	      (vec3+ pv r))))))

(define (get-parent-node-tree node)
  (cons node
	(if (has-parent-node node)
	    (get-parent-node-tree (get-parent-node node))
	    null)))

(define (find-common-parent nodea nodeb)
  (let ((ta (reverse (get-parent-node-tree nodea)))
	(tb (reverse (get-parent-node-tree nodeb))))
    (for/last ([a ta]
	       [b tb]
	       #:when (equal? a b))
	      a)))
...
(define (lookup-blocks node position size cb (idx -1))
  ...
  (when (and (not (eq? idx -2)) 
	     (has-parent-node node))
	(let ((idx2 (get-node-index node)))
	  (lookup-blocks (get-parent-node node) (to-parent-coords2 idx2 position) 
...
(define tile (sprite (read-bitmap "tile22.png") 0 -41));-37))
(define tile2 (sprite (read-bitmap "tile2x2.png") 0 -77));-75));(sprite (read-bitmap "tree.png") 0 -14))
...
(define (add-entity node entity)
  ...
  (let* ((size (entity-size entity))
	...
	(begin
	  ...
	  (add-entity (get-parent-node node) entity))
	  ...
	  (let ((new-node (get-relative-node node (vec3-apply exact-floor position))))
	    ...
	    (set-node-payload! new-node (cons entity (get-node-payload new-node)))
	    entity)))))

(define (remove-entity entity)
  (let ((node (entity-node entity)))
    (set-node-payload! node (remove entity (get-node-payload node)))
...
(define (remove-satelite sat)
  (let ((entity (satelite-entity-origin sat))
    ...
    (set-node-payload! node (remove sat (get-node-payload node)))
    ))
...
(define (add-satelite entity position node)
  (let ((sat (satelite-entity position entity node)))
    ...
    (set-node-payload! node (cons sat (get-node-payload node)))
    ))
...
;; (printf "C: ~a\n" c)
;; (printf "~a\n" (map (lambda (x) (eq? c x)) (get-parent-node-tree (entity-node l2))))
;; (printf "~a\n" (map (lambda (x) (eq? c x)) (get-parent-node-tree p1)))
;; (printf "eq: ~a\n" (eq? c  l2))
;; (print (get-parent-node-offset (entity-node l2) 1 c))
;; (newline)
;; (printf "Payload: ~a" (get-node-payload (entity-node l2)))
;(exit 0)

(define p2 (get-child-node p1 0))
(define p3 (get-child-node p1 4))

;(set-node-payload! p3 tile3)
(define p4 (get-child-node p2 0))
;(set-node-payload! p4 tile2)
(define p42 (get-child-node p2 4))
(set-node-payload! p42 (list tile2))
(define p43 (get-relative-node p42 (vec3 1 0 0)))
(set-node-payload! p43 (list tile2))
(define p44 (get-relative-node p43 (vec3 0 1 0)))
(set-node-payload! p44 (list tile5))
(define p5 (get-child-node p2 0))
(define n1 (get-child-node p5 2))
(define n2 (get-relative-node n1 (vec3 0 0 1)))
(set-node-payload! n1 (list tile)) 
(set-node-payload! n2 (list tile)) 

(define sprite-table (make-weak-hash))

(define p32 (get-relative-node p3 (vec3 1 0 0)))
(set-node-payload! p32 (list tile3))
(define p33 (get-relative-node p3 (vec3 1 0 -1)))
(define p35 (get-relative-node p3 (vec3 2 0 -1)))
(set-node-payload! p33 (list tile3))
(set-node-payload! p35 (list tile4))
(define p34 (get-relative-node p3 (vec3 1 0 -2)))
;(set-node-payload! p34 tile3)
...
(new my-canvas% [parent frame]
	...
	(let ((node (entity-node e1)))
	  ...
	  (let ((ep (entity-position e1))
		(es (entity-size e1))
		(n2 (map (curry get-relative-node node) (list (vec3 0 0 1) (vec3 0 0 -1) (vec3 1 0 0) (vec3 -1 0 0))))
		)
	      ...
	      (lookup-blocks node np (entity-size e1) 
			   (lambda (n p s) 
			     (begin
			       (when (and (equal? s es)
					  (not (equal? p ep)))
				     (add-satelite e1 p n) ;; this strat annot be used as it requires there to be a node which might not exist.
				     (printf "Collision with: ~a ~a ~a ~a\n" s es p ep))
			       (for ([i2 (get-node-payload n)]
				     #:when (sprite? i2))
				    (set! collides true))

			      ;(set-node-payload! n null)
	...
	(let* ((p (get-parent-node (get-parent-node (get-parent-node (get-parent-node (get-parent-node (get-parent-node (entity-node e1))))))))
	       (pv (get-parent-node-offset (entity-node e1) 24 p))
	       (o (iso-offset pv)))
	  (newline)
	  (render-node p (* 24 2 2 2 2 2 2)
		       (lambda (node xyz s)
			 (let ((im-v (iso-offset (vec3+ xyz pv (vec3-apply (lambda (x) (* x 24 -1)) (entity-position e1)))))
			       (tile (get-node-payload node)))
